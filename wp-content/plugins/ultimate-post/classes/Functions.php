...
class Functions{
    ...
    public function is_archive_child_builder() {
        ...
        if($builder_type) {
            ...
            $conditons = $settings[$builder_type][$id];
            $conditions_length = sizeof($conditons) > 1 ? 'multiple' : 'single';
            foreach ($conditons as $conditon) {
                if(strpos($conditon,'singular/front_page') > -1) {
    ...
    public function pagination($pages = '', $paginationNav = '', $paginationText = '', $paginationAjax = true, $baseUrl = '') {
        ...
        if (1 != $pages) {
            $html .= '<ul class="ultp-pagination">';            
                ...
                if ($pages > 4) {
                    $html .= '<li class="ultp-last-pages" '.($pages<=$paged+1?$display_none:"").' data-current="'.$pages.'"><a href="'.$this->generatePaginationUrl($pages, $baseUrl).'">'.$pages.'</a></li>';
                }
                if ($paged != $pages) {
    ...
    /**
	 * Get Page Post Id ( Kadence elemwnt )
     * 
     * @since v.2.8.9
	 * @return NULL
	 */
    public function get_page_post_id($page_post_id, $blockId) {
        global $wpdb;
        $post_meta = $wpdb->get_row($wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "postmeta WHERE meta_value LIKE %s", '%.ultp-block-'.$blockId.'%'));
